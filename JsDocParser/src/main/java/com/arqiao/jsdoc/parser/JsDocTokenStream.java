...
import com.arqiao.jsdoc.parser.util.TokenUtil;
...
public class JsDocTokenStream {
  ...
  JsDocTokenStream(String sourceString, int lineno, int initCharno) {
    Assert.checkNotNull(sourceString);
    this.lineno = lineno;
  ...
  final public JsDocToken getJsDocToken() {
    int c;
    stringBufferTop = 0;
    for (;;) {
      // eat white spaces
  ...
  final public int getLineno() { return lineno; }

  final public int getCharno() {
    return lineno == initLineno ? initCharno + charno : charno;
  }

  final public String getString() { return string; }
